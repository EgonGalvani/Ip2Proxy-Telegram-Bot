...
/**
 * @modify by Egon Galvani to match ProxyResult class 
 **/
...
*/
	public int IsProxy(String IP) throws IOException {
		return ProxyQuery(IP, Modes.IS_PROXY).getIs_Proxy();
	}
...
*/
	public String GetCountryShort(String IP) throws IOException {
		return ProxyQuery(IP, Modes.COUNTRY_SHORT).getCountry_Short();
	}
...
*/
	public String GetCountryLong(String IP) throws IOException {
		return ProxyQuery(IP, Modes.COUNTRY_LONG).getCountry_Long();
	}
...
*/
	public String GetRegion(String IP) throws IOException {
		return ProxyQuery(IP, Modes.REGION).getRegion();
	}
...
*/
	public String GetCity(String IP) throws IOException {
		return ProxyQuery(IP, Modes.CITY).getCity();
	}
...
*/
	public String GetISP(String IP) throws IOException {
		return ProxyQuery(IP, Modes.ISP).getISP();
	}
...
*/
	public String GetProxyType(String IP) throws IOException {
		return ProxyQuery(IP, Modes.PROXY_TYPE).getProxy_Type();
	}
	...
	public ProxyResult ProxyQuery(String IPAddress, Modes Mode) throws IOException {
		...
		try {
			if (IPAddress == null || IPAddress.length() == 0) {
				Result.setIs_Proxy(-1);
				Result.setProxy_Type(MSG_INVALID_IP);
				Result.setCountry_Short(MSG_INVALID_IP);
				Result.setCountry_Long(MSG_INVALID_IP);
				Result.setRegion(MSG_INVALID_IP);
				Result.setCity(MSG_INVALID_IP);
				Result.setISP(MSG_INVALID_IP);
				return Result;
			}
			...
			if (_DBType == 0) {
				if (!LoadBIN()) { // problems reading BIN
					Result.setIs_Proxy(-1);
					Result.setProxy_Type(MSG_MISSING_FILE);
					Result.setCountry_Short(MSG_MISSING_FILE);
					Result.setCountry_Long(MSG_MISSING_FILE);
					Result.setRegion(MSG_MISSING_FILE);
					Result.setCity(MSG_MISSING_FILE);
					Result.setISP(MSG_MISSING_FILE);
					return Result;
				}
			}
			...
			if (IPType == 4) { // IPv4
				...
				if (_DBCountIPv6 == 0) {
					Result.setIs_Proxy(-1);
					Result.setProxy_Type(MSG_IPV6_UNSUPPORTED);
					Result.setCountry_Short(MSG_IPV6_UNSUPPORTED);
					Result.setCountry_Long(MSG_IPV6_UNSUPPORTED);
					Result.setRegion(MSG_IPV6_UNSUPPORTED);
					Result.setCity(MSG_IPV6_UNSUPPORTED);
					Result.setISP(MSG_IPV6_UNSUPPORTED);
					return Result;
				}
			...
			while (Low <= High) {
				...
				if (IPNo.compareTo(IPFrom) >= 0 && IPNo.compareTo(IPTo) < 0) {
					...
					Result.setIs_Proxy(Is_Proxy);
					Result.setProxy_Type(Proxy_Type);
					Result.setCountry_Short(Country_Short);
					Result.setCountry_Long(Country_Long);
